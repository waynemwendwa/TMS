...
export default function ProjectDetailsPage() {
  ...
  const exportBoqTemplateToExcel = (boqTemplate: BoqTemplate) => {
    try {
      const worksheetData = [
        ['BOQ Template Export'],
        ['Title:', boqTemplate.title],
        ['Equipment Installation Works:', boqTemplate.equipmentInstallationWorks || ''],
        ['Bill Number:', boqTemplate.billNumber || ''],
        ['Created By:', boqTemplate.createdByUser?.name || ''],
        ['Created At:', new Date(boqTemplate.createdAt).toLocaleString()],
        [],
        ['Item', 'Description', 'Quantity', 'Unit', 'Rate', 'Amount']
      ];

      boqTemplate.items.forEach(item => {
        worksheetData.push([
          item.item || '',
          item.description || '',
      ...
      // Calculate total
      const total = boqTemplate.items.reduce((sum, item) => sum + item.amount, 0);
      worksheetData.push([]);
      worksheetData.push(['', '', '', '', 'Total:', total.toString()]);
      ...
      XLSX.writeFile(workbook, `BOQ_${boqTemplate.title}_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error) {
      console.error('Error exporting BOQ to Excel:', error);
      alert('Failed to export BOQ to Excel. Please try again.');
    }
  };

  const exportOrderTemplateToExcel = (orderTemplate: OrderTemplate) => {
    try {
      const worksheetData = [
        ['Order Template Export'],
        ['Title:', orderTemplate.title],
        ['Created By:', orderTemplate.createdByUser?.name || ''],
        ['Created At:', new Date(orderTemplate.createdAt).toLocaleString()],
        [],
        ['Item', 'Description', 'Quantity', 'Unit', 'Rate', 'Amount']
      ];
      ...
      // Calculate total
      const total = orderTemplate.items.reduce((sum, item) => sum + item.amount, 0);
      worksheetData.push([]);
      worksheetData.push(['', '', '', '', 'Total:', total.toString()]);
      ...
      XLSX.writeFile(workbook, `Order_${orderTemplate.title}_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error) {
      console.error('Error exporting Order to Excel:', error);
      alert('Failed to export Order to Excel. Please try again.');
    }
  };
